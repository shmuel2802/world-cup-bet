...
app.post('/api/bets', authenticateToken, (req, res) => {
  ...
  if (match.status !== 'SCHEDULED') {
    return res.status(400).json({ message: 'לא ניתן לשלוח הימור. המשחק כבר החל או הסתיים.' });
  }

  const now = new Date();
  if (now >= new Date(match.utcDate)) {
    return res.status(400).json({ message: 'לא ניתן לשלוח הימור. המשחק כבר משוחק.' });
  }
  ...
  if (existingBet) {
    ...
    existingBet.predictedHomeScore = betType === 'EXACT_SCORE' ? parseInt(predictedHomeScore) : null;
    existingBet.predictedAwayScore = betType === 'EXACT_SCORE' ? parseInt(predictedAwayScore) : null;
...
app.delete('/api/bets/:betId', authenticateToken, (req, res) => {
  ...
  if (match.status !== 'SCHEDULED') {
    return res.status(400).json({ message: 'לא ניתן לבטל הימור על משחק שהתחיל או הסתיים.' });
  }

  const now = new Date();
  if (now >= new Date(match.utcDate)) {
    return res.status(400).json({ message: 'לא ניתן לבטל הימור לאחר שעת התחלת המשחק.' });
  }
